...
class ClimatePhenoDataset(Dataset):
    def __init__(
        ...
        self.stats_year_temp = (
            ...
            np.array(list(self.year_temp.values())).std()
        )
        bins = np.arange(6, 10, 0.3) # 14+1 classes
        self.cls_year_temp = {}
        for k, v in self.year_temp.items():
            self.cls_year_temp[k] = np.digitize(v, bins, right=False)

        # Load M1 predictions
        if load_M1_info is not None:
            file = [f for f in os.listdir(load_M1_info["pred_folder"]) if f.endswith(".csv") and load_M1_info["split"] in f and f"fold_{load_M1_info['fold']}" in f]
            if len(file) > 1:
                raise f"more than one files in M1 prediction folder {file}"
            else:
                print(f"loading M1 predictions from {file}")
            self.M1_preds = pd.read_csv(f"{load_M1_info['pred_folder']}/{file[0]}")
            self.M1_preds = self.M1_preds.set_index("site_years")
    ...
    def __getitem__(self, item, return_siteyear=False):
        ...
        if self.M1_preds is not None:
            if self.site_years[item] in self.M1_preds.index:
                m1preds = self.M1_preds.loc[self.site_years[item]][self.target_list]
            else:
                m1preds = self.M1_preds[self.target_list].mean()
            if self.normalise_dates:
                m1preds = m1preds.fillna(self.nan_value_target).to_dict()
                for k in self.target_list:
                    if m1preds[k] != self.nan_value_target:
                        m, s = self.target_scaler[k]
                        m1preds[k] = (m1preds[k] - m) / s
            else:
                m1preds = m1preds.fillna(self.nan_value_target).to_dict()
            out["M1_preds"] = m1preds 
            # Convert to tensor
            m1_tensor = [m1preds[k] for k in self.target_list]
            out["M1_tensors"] = m1_tensor # [t, 1]
        ...
        out["year_normalised"] = (
            out["year"] - self.stats_year[0]
        ) / self.stats_year[1]

        out["year_temp"] = self.year_temp[str(out["year"])]
        out["year_temp_normalised"] =  out["year_temp"]
        (
            out["year_temp"] - self.stats_year_temp[0]
        ) / self.stats_year_temp[1]

        out["cls_year_temp"] = self.cls_year_temp[str(out["year"])]
